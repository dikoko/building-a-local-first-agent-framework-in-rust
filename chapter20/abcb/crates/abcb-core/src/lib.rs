...
/// later be injected (P4-T04 discussion) — not yet filled.
...
/// policy in tests, and eventually an interactive prompt or allow-list at P5-T08
...
pub async fn run_step(
    ...
    // parse (fenced/garbage JSON — the P4-T07 failure mode) is still recorded.
    write_event(
        events,
        &Event::ModelResponse {
...
/// the session's id (P4-T02) and inspect the session after an error — the
...
pub async fn run_loop(
    ...
    // these (P4-T02 contract), so they're the run's starting point. Scanning for
