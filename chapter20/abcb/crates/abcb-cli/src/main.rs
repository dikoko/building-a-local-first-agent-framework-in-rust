use abcb_core::{
    AllowAll, ApprovalDecision, ApprovalPolicy, Event, LoggedEvent, LoopError, Message,
    MockProvider, Provider, Role, Session, ToolRegistry, one_turn, read_events, run_loop,
    system_prompt, write_event,
};
use abcb_models::{OpenAiCompatProvider, check_health};
use abcb_tools::{
    AddNumbers, Echo, ListDir, ReadFile, RunCommand, SessionNoteAppend, SessionNoteSearch,
};
#[cfg(test)]
...
struct ModelConfig {
    ...
    model: String,
    /// Optional health endpoint used by `abcb doctor` (P3-T03).
...
/// The safe tool set, always available. Notably *excludes* `run_command`, which
/// is registered only on the interactive `run` path (where a human can approve
/// it) — so unattended callers like `eval` can never execute a shell command.
/// The file tools confine reads to `project_root`.
...
/// An [`ApprovalPolicy`] that auto-allows a known-safe set of tools and prompts
/// the user (on stderr, reading stdin) for anything else.
///
/// The set is an allow-list, not a deny-list, on purpose: the default for an
/// unlisted tool is *prompt*, not *allow*, so a tool the author forgot to
/// classify fails toward asking rather than silently running.
...
impl ApprovalPolicy for InteractivePolicy {
    fn approve(&mut self, tool_name: &str, arguments: &serde_json::Value) -> ApprovalDecision {
        ...
        // Prompt on stderr (stdout is reserved for the final answer) and read a
        // line from stdin. A read error or EOF leaves the answer empty, which
        // `decide_from_answer` treats as No — the gate fails safe.
...
/// Pure decision over a typed answer: Allow only on an explicit `y`/`yes` (any
/// case). Everything else — including empty input from EOF — is Deny.
...
///
/// Pure path composition — no I/O — so it's directly unit-testable.
...
async fn run_run(message: String, mock: bool) -> Result<(), Box<dyn Error>> {
    // The CLI runs in the project directory, so "." is the project root the file
    // tools confine to and run_command's working directory.
    let project_root = PathBuf::from(".");
    let mut registry = default_registry(&project_root, PathBuf::from(NOTES_PATH));
    // run_command is the one tool that can affect the system, so it's wired only
    // here (the interactive path) and gated by InteractivePolicy on the real run.
    registry
    ...
    let (outcome, endpoint, events_path) = if mock {
        ...
        // The real run is interactive: gate every non-safe tool (run_command)
        // behind a y/N prompt. The mock path stays AllowAll — it's the
        // deterministic, no-input path and never reaches a tool call.
        let outcome = run_loop(
            &mut provider,
            &registry,
            &mut session,
...
/// with no change here. `append-note` is the probe that answers the deferred
/// P4-T07 question — how reliably does the model emit a correct tool call?
...
async fn run_eval_command(runs: usize) -> Result<(), Box<dyn Error>> {
    ...
    // eval runs in the project dir; the safe registry has no run_command, so an
    // unattended eval can never shell out regardless of what the model emits.
...
mod tests {
    ...
    fn default_registry_has_the_safe_tools_but_not_run_command() {
        ...
        // run_command is deliberately absent — it's wired only on the
        // interactive run path, never in this unattended default set.
        assert_eq!(
            names,
            vec![
                "add_numbers",
    ...
    fn interactive_policy_auto_allows_safe_tools_without_prompting() {
        ...
        // These short-circuit before any stdin read, so the test never blocks.
    ...
    fn decide_from_answer_allows_only_explicit_yes() {
        ...
        // No, blank (EOF), and anything ambiguous all fail safe to Deny.
    ...
    async fn run_loop_with_real_tools_appends_a_note_then_finishes() {
        ...
        let registry = default_registry(Path::new("."), notes.path().to_path_buf());
