use abcb_core::{
    Event, Message, MockProvider, Role, Session, ToolRegistry, one_turn, read_events, run_loop,
    write_event,
};
...
async fn run_run(message: String, mock: bool) -> Result<(), Box<dyn Error>> {
    ...
    // The CLI owns the session: stamp it, store it, then drive it. Storage is a
    // property of the session, not of which provider answers — so both the mock
    // and real paths create the directory; only the storage root and step
    // ceiling differ (mock has no config to read them from).
...
/// Open `<session-dir>/events.jsonl` for appending, buffered.
///
...
mod tests {
    ...
    async fn run_loop_with_real_tools_appends_a_note_then_finishes() {
        ...
        let answer = run_loop(&mut provider, &registry, &mut session, 5, &mut io::sink())
            .await
            .expect("loop should finish");

        assert_eq!(answer, "noted");
        // The real session_note_append tool ran through the loop and wrote to disk.
