...
mod tests {
    ...
    async fn run_step_returns_final_for_final_envelope() {
        ...
        let outcome = run_step(&mut provider, &registry, &mut session, &mut io::sink())
            .await
    ...
    async fn run_step_appends_assistant_and_tool_messages_to_session() {
        ...
        run_step(&mut provider, &registry, &mut session, &mut io::sink())
            .await
            .expect("step should succeed");
    ...
    async fn run_step_errors_on_unknown_tool() {
        ...
        let err = run_step(&mut provider, &registry, &mut session, &mut io::sink())
            .await
