...
use abcb_models::OpenAiCompatProvider;
...
impl Config {
    ...
    // Consumed when sessions get their own directories at P4-T02; tested now.
    #[allow(dead_code)]
...
async fn run_run(message: String, mock: bool) -> Result<(), Box<dyn Error>> {
    ...
    let answer = if mock {
        ...
        run_loop(&mut provider, &registry, &message, DEFAULT_MAX_STEPS).await?
    } else {
        let config = load_required_config()?;
        let mut provider = build_provider(&config)?;
        run_loop(&mut provider, &registry, &message, config.max_steps()).await?
    };
    println!("{answer}");
...
mod tests {
    ...
    async fn run_loop_with_real_tools_appends_a_note_then_finishes() {
        ...
        let answer = run_loop(&mut provider, &registry, "remember to buy milk", 5)
            .await
