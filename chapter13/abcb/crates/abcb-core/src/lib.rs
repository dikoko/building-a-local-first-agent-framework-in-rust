...
///
/// The lone clock read in the session path. `duration_since` returns a `Result`
/// because the system clock *can* be set before the epoch; that case is not
/// something a caller can act on, so we treat it as `0` rather than propagate.
fn now_millis() -> u64 {
...
/// share a process — e.g. the Phase 5 daemon.
...
mod tests {
    ...
    async fn run_loop_returns_final_immediately() {
        ...
        let answer = run_loop(&mut provider, &registry, &mut session_with_user("hi"), 5)
            .await
    ...
    async fn run_loop_errors_when_max_steps_exceeded() {
        ...
        let err = run_loop(&mut provider, &registry, &mut session_with_user("hi"), 2)
            .await
    ...
    async fn run_loop_aborts_on_provider_error() {
        ...
        let err = run_loop(&mut provider, &registry, &mut session_with_user("hi"), 5)
            .await
